...
object MultiFetchResponse {
  ...
  object Result {
    ...
    private given Encoder.AsObject[Error] = {
    ...
    given JsonLdEncoder[Error] = JsonLdEncoder.computeFromCirce(ContextValue(contexts.error))

    private given JsonLdApi = TitaniumJsonLdApi.lenient

    private[model] def itemEncoder(repr: ResourceRepresentation)(using BaseUri, RemoteContextResolution) = {
      (item: Result) =>
        {
          ...
          def valueToJson[A](content: JsonLdContent[A]): IO[Json] = {
            given JsonLdEncoder[A] = content.encoder
