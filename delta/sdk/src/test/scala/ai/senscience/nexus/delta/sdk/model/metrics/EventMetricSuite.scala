...
import ai.senscience.nexus.delta.sdk.model.metrics.EventMetricSuite.SimpleEvent
...
class EventMetricSuite extends NexusSuite {

  test("A metric should be correctly created from the event") {
    ...
    val expected = ProjectScopedMetric(
      Instant.EPOCH,
      Anonymous,
      2,
    ...
    assertEquals(metric.asJson, expectedJson)
  }

}

object EventMetricSuite {
