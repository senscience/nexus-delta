...
class RealmsNextSuite extends NexusSuite with OptionAssertions {
  ...
  test("Updating a realm returns an updated state") {
    // format: off
    val expected = RealmState(label, 2, deprecated = false, name, wellKnown2Uri, wk2.issuer, wk2.grantTypes, None, None, wk2.authorizationEndpoint, wk2.tokenEndpoint, wk2.userInfoEndpoint, wk2.revocationEndpoint, wk2.endSessionEndpoint, epoch, Anonymous, time2, subject)
    // format: on
