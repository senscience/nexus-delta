...
final class Serializer[Id, Value] private (
  ...
  def putValue: Put[Value] = jsonbPut(using jsonIterCodec).contramap(codec(_))
}
...
object Serializer {
  ...
  /**
    ...
    */
  def apply[Id, Value](extractId: Id => Iri)(using codec: Codec.AsObject[Value]): Serializer[Id, Value] =
  ...
  /**
    ...
    */
  def apply[Value]()(using codec: Codec.AsObject[Value]): Serializer[Iri, Value] =
    apply(identity[Iri])(using codec)

  /**
    * Defines a serializer with the default printer ignoring null values with a custom [[Id]]
    */
  def dropNulls[Id, Value](extractId: Id => Iri)(using codec: Codec.AsObject[Value]): Serializer[Id, Value] =
  ...
  def dropNullsInjectType[Id, State <: ScopedState](
      extractId: Id => Iri
  )(using codec: Codec.AsObject[State]): Serializer[Id, State] = {
    ...
    dropNulls(extractId)(using codecWithType)
  }

  /**
    * Defines a serializer with the default printer ignoring null values with an [[Iri]] id
    */
  def dropNulls[Value]()(using codec: Codec.AsObject[Value]): Serializer[Iri, Value] =
    dropNulls(identity[Iri])(using codec)

  /**
    * Defines a serializer for states with the default printer ignoring null values with an [[Iri]] id and injecting the
    * resource types
    */
  def dropNullsInjectType[State <: ScopedState]()(using codec: Codec.AsObject[State]): Serializer[Iri, State] =
    dropNullsInjectType(identity[Iri])(using codec)
