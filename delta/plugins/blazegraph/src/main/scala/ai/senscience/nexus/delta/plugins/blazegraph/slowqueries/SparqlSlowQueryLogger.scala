...
trait SparqlSlowQueryLogger {
  ...
  def search(pagination: Pagination, timeRange: TimeRange): IO[SparqlSlowQueryResults]

  def save[A](view: ViewRef, sparql: SparqlQuery, subject: Subject, io: IO[A]): IO[A]
}
...
object SparqlSlowQueryLogger {
  ...
  def noop: SparqlSlowQueryLogger = new SparqlSlowQueryLogger {
    ...
    override def search(pagination: Pagination, timeRange: TimeRange): IO[SearchResults[SparqlSlowQuery]] =
      IO.pure(SearchResults(0, List.empty))

    override def save[A](view: ViewRef, sparql: SparqlQuery, subject: Subject, io: IO[A]): IO[A] = io
  }

  def apply(store: SparqlSlowQueryStore, longQueryThreshold: Duration, clock: Clock[IO]): SparqlSlowQueryLogger =
    new SparqlSlowQueryLogger {

      override def search(pagination: Pagination, timeRange: TimeRange): IO[SearchResults[SparqlSlowQuery]] =
        IO.pure(SearchResults(0, List.empty))

      override def save[A](view: ViewRef, sparql: SparqlQuery, subject: Subject, io: IO[A]): IO[A] = {
